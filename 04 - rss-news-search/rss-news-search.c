...
static void BuildIndices(const char *feedsFileName, hashset *stopList, hashset *relatedArticlesSet);
static void ProcessFeed(const char *remoteDocumentName, hashset *stopList, hashset *relatedArticlesSet);
static void PullAllNewsItems(FILE *dataStream, hashset *stopList, hashset *relatedArticlesSet);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *stopList,
                                    hashset *relatedArticlesSet, vector* indexedArticles);
...
static void ParseArticle(const char *articleTitle, const char *articleURL,
                          hashset *stopList, hashset *relatedArticlesSet);
static void ScanArticle(streamtokenizer *st, const char *articleTitle, const char *articleURL,
                          hashset *stopList, hashset *relatedArticlesSet);
static void QueryIndices(hashset *stopList, hashset *relatedArticlesSet);
static void ProcessResponse(const char *word, hashset *stopList, hashset *relatedArticlesSet);
...
static const char *const kStopListFile = "data/stop-words.txt";
static const char *const kFilePrefix = "file://";
static const char *const kTextDelimiters =
    " \t\n\r\b!@$%^*()_+={[}]|\\'\":;/?.>,<~`";

static const int kPrime = 1009;
static const int kBiggerPrime = 10007;
...
void articleAndWordCountNew(articleAndWordCount *article, const char* articleName, const char* articleLink) {
  article -> name = strdup(articleName);
  article -> link = strdup(articleLink);
  article -> count = 1;
}

int articleAndWordCountCmp(const void *elem1Addr, const void* elem2Addr) {
  articleAndWordCount* elem1Ptr = (articleAndWordCount*)elem1Addr;
  articleAndWordCount* elem2Ptr = (articleAndWordCount*)elem2Addr;
  return strcasecmp(elem1Ptr -> link, elem2Ptr -> link);
}

int articleAndWordCountHash(const void *elemAddr, int numBuckets) {
  articleAndWordCount* elemPtr = (articleAndWordCount*)elemAddr;
  return StringHash(elemPtr -> link, numBuckets);
}

void articleAndWordCountDispose(void* awc) {
  free(((articleAndWordCount*)awc) -> name);
  free(((articleAndWordCount*)awc) -> link); 
}
...
void relatedArticlesNew(relatedArticles *ra, const char* curWord) {
  ra -> word = strdup(curWord);
  hashset articlesSet;
  HashSetNew(&articlesSet, sizeof(articleAndWordCount), kPrime,
                articleAndWordCountHash, articleAndWordCountCmp, articleAndWordCountDispose);
  ra -> articles = articlesSet;
}

int relatedArticlesCmp(const void *elem1, const void* elem2) {
  return strcasecmp(((relatedArticles*)elem1) -> word, ((relatedArticles*)elem2) -> word);
}

int relatedArticlesHash(const void *elemAddr, int numBuckets) {
  relatedArticles* elemPtr = (relatedArticles*)elemAddr;
  return StringHash(elemPtr -> word, numBuckets);
}

void relatedArticlesFree(void* elemAddr) {
  relatedArticles *elemPtr = (relatedArticles*) elemAddr;
  free(elemPtr -> word);
  HashSetDispose(&(elemPtr->articles)); 
}


int strcasecmpNew(const void *str1, const void *str2) {
  return strcasecmp(*(const char**)str1, *(const char**)str2);
}
int stopWordHash(const void *elem, int numBuckets) {
  return StringHash(*(const char**) elem, numBuckets);
}
void stopListDispose(void* elemAddr) {
  free(*(char**)elemAddr);
}

static const char *const kNewLineDelimiters = "\r\n";
static void initStopList(hashset* stopList) {
  HashSetNew(stopList, sizeof(char*), kPrime, stopWordHash, strcasecmpNew, stopListDispose);
  streamtokenizer stopTokens;
  FILE* stopListFile = fopen(kStopListFile, "r");
  assert(stopListFile != NULL);
  STNew(&stopTokens, stopListFile, kNewLineDelimiters, true);
  char word[128];
  while (STNextToken(&stopTokens, word, sizeof(word))) {
    char* dummy = strdup(word);
    HashSetEnter(stopList, &dummy);
  }
...
int main(int argc, char **argv) {
  hashset stopList, relatedArticlesSet;
  initStopList(&stopList);
  HashSetNew(&relatedArticlesSet, sizeof(relatedArticles), kBiggerPrime, relatedArticlesHash, relatedArticlesCmp, relatedArticlesFree);
  
  setbuf(stdout, NULL);
  curl_global_init(CURL_GLOBAL_DEFAULT);
  Welcome(kWelcomeTextFile);
  BuildIndices(((argc == 1) ? kDefaultFeedsFile : argv[1]), &stopList, &relatedArticlesSet);
  QueryIndices(&stopList, &relatedArticlesSet);

  curl_global_cleanup();
  HashSetDispose(&stopList);
  HashSetDispose(&relatedArticlesSet);
  return 0;
}
...
static void BuildIndices(const char *feedsFileName, hashset *stopList, hashset *relatedArticlesSet) {
  ...
  while (STSkipUntil(&st, ":") !=
    ...
    ProcessFeed(remoteFileName, stopList, relatedArticlesSet);
  }
...
static void ProcessFeedFromFile(char *fileName, hashset *stopList, hashset *relatedArticlesSet) {
  FILE *infile;
  streamtokenizer st;
  infile = fopen((const char *)fileName, "r");
  assert(infile != NULL);
  STNew(&st, infile, kTextDelimiters, true);
  ScanArticle(&st, (const char *)fileName, (const char *)fileName, stopList, relatedArticlesSet);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew
                  // doesn't open one..
  fclose(infile);
}
...
static void ProcessFeed(const char *remoteDocumentName, hashset *stopList, hashset *relatedArticlesSet) {
  ...
  if (!strncmp(kFilePrefix, remoteDocumentName, strlen(kFilePrefix))) {
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(kFilePrefix), stopList, relatedArticlesSet);
    return;
  }

  FILE *tmpFeed = FetchURL(remoteDocumentName, "tmp_feed");
  PullAllNewsItems(tmpFeed, stopList, relatedArticlesSet);
  fclose(tmpFeed);
}
...
void articleNew(article *a, char* name, char* URL) {
  url u;
  URLNewAbsolute(&u, URL);
  a->title = strdup(name);
  a->link = u;
}

void articleDispose(void* a) {
  free(((article*)a) -> title);
  URLDispose(&((article*)a)->link);
}

int articleCmp(const void* elem1, const void* elem2) {
  article* article1 = (article*)elem1;
  article* article2 = (article*)elem2;
  int resMaybe = strcasecmp(article1->link.fullName, article2->link.fullName);
  if(strcmp(article1->title, article2->title)==0  &&
          strcmp(article1->link.serverName, article2->link.serverName)==0) return 0;
  return resMaybe;
}
...
static void PullAllNewsItems(FILE *dataStream, hashset *stopList, hashset *relatedArticlesSet) {
  ...
  VectorNew(&indexedArticles, sizeof(article), articleDispose, 40);

  while (GetNextItemTag(
      &st)) { // if true is returned, then assume that <item ...> has just been
              // read and pulled from the data stream
    ProcessSingleNewsItem(&st, stopList, relatedArticlesSet, &indexedArticles);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *stopList,
  ...
  article a;
  articleNew(&a, articleTitle, articleURL);
  int isParsed = VectorSearch(indexedArticles, &a, articleCmp, 0, false);
  if(isParsed == -1) {
    VectorAppend(indexedArticles, &a);
    ParseArticle(articleTitle, articleURL, stopList, relatedArticlesSet);
  } else articleDispose(&a);
}
...
static void ParseArticle(const char *articleTitle,
  ...
  int response = urlConn.responseCode;
  if(response == 200) {
    printf("Scanning \"%s\"\n", articleTitle);
    streamtokenizer st;
    STNew(&st, tmpDoc, kTextDelimiters, false);
    ScanArticle(&st, articleTitle, articleURL, stopList, relatedArticlesSet);
    STDispose(&st);
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWellFormed(word)) {
        ...
        char* dummy = word;
        if(HashSetLookup(stopList, &dummy) != NULL) continue;

        articleAndWordCount awc;
        articleAndWordCountNew(&awc, articleTitle, articleURL);
        relatedArticles* ptr = HashSetLookup(relatedArticlesSet, &dummy);
        if(ptr == NULL) {
          relatedArticles ra;
          relatedArticlesNew(&ra, word);
          HashSetEnter(&ra.articles, &awc);
          HashSetEnter(relatedArticlesSet, &ra);
        } else {
          articleAndWordCount* ptr2 = HashSetLookup(&ptr->articles, &awc);
          if(ptr2 != NULL) {
            ptr2 -> count++;
            articleAndWordCountDispose(&awc);
          } else {
            HashSetEnter(&ptr->articles, &awc);
          }
        }
      }
    }
...
static void QueryIndices(hashset *stopList, hashset *relatedArticlesSet) {
  ...
  while (true) {
    ...
    if (strcasecmp(response, "") == 0)
      break;
    ProcessResponse(response, stopList, relatedArticlesSet);
  }
}
...
int articleCmpForSorting(const void *elemAddr1, const void *elemAddr2) {
  return ((articleAndWordCount*)elemAddr1)->count - ((articleAndWordCount*)elemAddr2)->count;
}

void buildVector(void* elemAddr, void* vec) {
  VectorAppend((vector*)vec, elemAddr);
}

void SortPrint(hashset* articles) {
  vector articleVector;
  int size = HashSetCount(articles);
  VectorNew(&articleVector, sizeof(articleAndWordCount), NULL, size);
  HashSetMap(articles, buildVector, &articleVector);
  VectorSort(&articleVector, articleCmpForSorting);
  for(int i = size - 1; i >= 0; i--) {
    articleAndWordCount* curArticle = VectorNth(&articleVector, i);
    if(curArticle->count > 1) {
      printf("%d.) \"%s\" [search term occurs %d times]\n    \"%s\"\n",
              size-i, curArticle->name, curArticle->count, curArticle->link);
    } else {
      printf("%d.) \"%s\" [search term occurs %d time]\n    \"%s\"\n",
              size-i, curArticle->name, curArticle->count, curArticle->link);
    }
  }
  VectorDispose(&articleVector);
}
...
static void ProcessResponse(const char *word, hashset *stopList, hashset *relatedArticlesSet) {
  if(WordIsWellFormed(word)) {
    ...
    relatedArticles* ptr = HashSetLookup(relatedArticlesSet, &word);
    if(ptr == NULL) {
      printf("None of today's news articles contain the word \"%s\".\n", word);
      return;
    } else {
      SortPrint(&ptr->articles);
    }
  } else {
