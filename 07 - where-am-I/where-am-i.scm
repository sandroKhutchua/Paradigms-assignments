...
(define (partition pivot point-list)
  (if (null? point-list) '(() ())
      (let ((split-of-rest (partition pivot (cdr point-list))))
...
(define (quicksort point-list)
  (if (<= (length point-list) 1) point-list
      (let ((split (partition (car point-list) (cdr point-list))))
...
(define (intersection-points-helper circle others)
  (if (null? others) '()
      (append (intersect circle (car others))
              (intersection-points-helper circle (cdr others)))
  )
)

(define (intersection-points circles)
  (if (< (length circles) 2) '()
      (append (intersection-points-helper (car circles) (cdr circles))
              (intersection-points (cdr circles)))
  )
)

(define (distance-product point others)
  (if (null? others) 1
      (apply * (map (lambda (x) (if (equal? point x) 1 (dist point x))) others))
  )
)
...
(define (clumped-points points)
  (map (lambda (point) (cadr point)) 
       (prefix-of-list (sort-points (rate-points points))
                       (/ (- (length points) (remainder (length points) 2)) 2))
  )
)

(define (average-point points)
  (let ((average (make-pt (/ (apply + (map (lambda (point) (x point)) points)) (length points))
                          (/ (apply + (map (lambda (point) (y point)) points)) (length points)))))
    (list (distance-product average points) average)
  )
)
